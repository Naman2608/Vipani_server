...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const latestTransactionsPromise = Order.find({})
            ...
            .limit(4);
        const [currentMonthProducts, currentMonthUsers, currentMonthOrders, lastMonthProducts, lastMonthUsers, lastMonthOrders, productsCount, usersCount, allOrders, lastSixMonthOrders, categories, femaleUsersCount, latestTransactions,] = await Promise.all([
...
export const getPieCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const totalProductionCost = allOrders.reduce((prev, order) => prev + (order.shippingCharges || 0), 0);
        const totalBurnt = allOrders.reduce((prev, order) => prev + (order.tax || 0), 0);
        const totalMarketingCost = Math.round(totalGrossIncome * (30 / 100));
        const netMargin = totalGrossIncome -
            totalDiscount -
            totalProductionCost -
            totalBurnt -
            totalMarketingCost;
